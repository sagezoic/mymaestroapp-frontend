...
function PostCard({ Feed, Refresh }) {
  ...
  useEffect(() => {
    const searchTerm = curUserId;

    if (likePostList != null) {
      const result = likePostList.filter(
        (object) => object.userId === searchTerm
      );
  ...
  const getLikePost = (postId) => {
    ...
    // formDataUpload.append("postId",formData.postId);
    axios
      .get(config.serverURL + `/likepost/getpostlike/${curUserId}`)
      .then((response) => {
        console.log(response);
        setLike(likePostList);
        console.log(like);
      })
      .catch((error) => {
        console.log(error);
  ...
  const deleteLike = (postId) => {
    formData.postId = postId;
    axios
      .delete(config.serverURL + `/likepost/delete`, formData)
      .then((response) => {
        console.log(response);
        setLike(false);
        console.log(like);
  ...
  const calcNoOfDays = (timeStamp) => {
    ...
    if (daysDifference > 1) {
      ...
      if (time > 0) {
        return time + " hr";
      } else return Math.floor(timeDifference / (1000 * 60)) + " mins";
    }
  };
  ...
  const deletePost = () => {
    ...
    axios
      .delete(
        config.serverURL +
          `/post/deletepost?userId=${sessionStorage.getItem("userId")}&postId=${
            Feed.id
          }`
      )
      .then((response) => {
        ...
        else history.push("/maestro/feed");
        Refresh();
      })
      .catch((error) => {
        console.log(error);
    ...
    <div className="card mb-3">
        ...
        <div className="row">
          <div className="col-md-1 me-3">
            <img
              src={
                postUser != null
                  ? config.serverURL + "/users/finddp?path=" + postUser.dpUrl
                  : ""
              }
          ...
          <div className="col-md-10">
            ...
            >
              {calcNoOfDays(Feed.timeStamp)} Ago{" "}
              <i className="fas fa-globe-americas fa-sm"></i>
            </span>
          </div>
        </div>
      </div>
      <div>
        ...
        <div className="card-footer bg-white mt-1">
          <span>
            <button
              className={`ref btn btn-light btn-sm`}
              onClick={
                like
                  ? () => {
                      deleteLike(Feed.id);
            ...
            >
              <i
                className="far fa-thumbs-up fa-md"
                style={{ color: `${like ? "blue" : ""}` }}
              >
                <span
                  className="ms-2 mediatext"
                  style={{ color: `${like ? "blue" : ""}` }}
                >
                  Like
                </span>
          ...
          <span>
            <button className="ref btn btn-light btn-sm">
              <i
                className="far fa-comment fa-md"
